...
def create_schema(
    ...
    for i in range(1, MAX_MODES + 1):
        ...
        sub_schema[vol.Optional(name_key, default=defaults.get(name_key, ""))] = cv.string
        
        if default_watt is not None:
            sub_schema[vol.Optional(watt_key, default=float(default_watt))] = selector.NumberSelector(num_config)
        else:
