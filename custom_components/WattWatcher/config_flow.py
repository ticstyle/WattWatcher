...
def create_schema(
    ...
    for i in range(1, MAX_MODES + 1):
        schema[vol.Optional(f"mode_{i}_name", default=defaults.get(f"mode_{i}_name", ""))] = cv.string
        schema[vol.Optional(f"mode_{i}_max_watt", default=defaults.get(f"mode_{i}_max_watt"))] = vol.Any(
        ...
        if user_input is not None:
            ...
            if error := validate_modes(user_input):
                ...
                return self.async_create_entry(
                    ...
                    options_id=f"wattwatcher_{slug}",
                )
